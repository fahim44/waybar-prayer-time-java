...
public class PrayTime {
    ...
    private int nearestPrayerIndex;
    ...
    private ArrayList<String> getDatePrayerTimes(int year, int month, int day,
                                                 double latitude, double longitude, double tZone, double currentTime) {
    ...
    private ArrayList<String> getPrayerTimes(Calendar date, double latitude,
                                             double longitude, double tZone, double currentTime) {

        int year = date.get(Calendar.YEAR);
        int month = date.get(Calendar.MONTH);
        int day = date.get(Calendar.DATE);

        return getDatePrayerTimes(year, month+1, day, latitude, longitude, tZone, currentTime);
    }
    ...
    private ArrayList<String> computeDayTimes() {
        ...
        this.nearestPrayerIndex = nearestTimeIndex(times);

        return adjustTimesFormat(times);
    }

    private int nearestTimeIndex(double[] times) {
        int index = 0;

        for (int i=0; i < times.length; i++) {
            if (i==1 || i==4) { // sunrise, sunset
                continue;
            }

            if (i==times.length - 1) { //isha, so just print this for now...
                index = i;
                break;
            }
    ...
    public static void main(String[] args) {
        double latitude = 23.780753;
        double longitude = 90.2548801;
        double timezone = 6;
        ...
        var currHourMinStr = currentHour + "." + currentMinute;
        var currHourMinDouble = Double.parseDouble(currHourMinStr);

        ArrayList<String> prayerTimes = prayers.getPrayerTimes(cal,
                latitude, longitude, timezone, currHourMinDouble);
        ArrayList<String> prayerNames = prayers.getTimeNames();

        StringBuilder resultStr = new StringBuilder("{\"text\":\"");
        resultStr.append(prayerNames.get(prayers.getNearestPrayerIndex()))
                .append("-")
                .append(prayerTimes.get(prayers.getNearestPrayerIndex()))
                .append("\",\"tooltip\":\"");

        for (int i = 0; i < prayerTimes.size(); i++) {
            resultStr.append(getFormattedTime(prayerNames.get(i), prayerTimes.get(i)));
        }
    ...
    private static String getFormattedTime(String prayerName, String time) {
    ...
    public int getNearestPrayerIndex() {
        return nearestPrayerIndex;
    }
}
